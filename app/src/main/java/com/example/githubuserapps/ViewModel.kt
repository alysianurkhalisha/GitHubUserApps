...
class ViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        val userDAO:UserDAO = UserDatabase.getDatabase(application.applicationContext).userDAO()
    ...
    fun searchUser(query: String) {
        ...
        request.searchUser(query).enqueue(object : Callback<Keluaran> {

            override fun onResponse(call: Call<Keluaran>, response: Response<Keluaran>) {
                users.postValue(response.body()!!.result)
                Log.d("onResponse", "searchUser: ${response.body()!!.result.size} - ${users.value?.size}")
            }

            override fun onFailure(call: Call<Keluaran>, t: Throwable) {
